...
export async function getSoftwareBySlug(slug) {
    ...
    const { data: software, error } = await supabase
        ...
        .select(`
        *, 
      software_suitability:software_software_suitability(
      description,
      is_suitable,
      software_suitability(*)
    ),
...
export async function getSupportedShopSystemsForSoftware(software) {
    ...
    const { data, error } = await supabase
      ...
      name,
      logo_url,
      software_integration!software_integration_integrated_with_id_fkey(integration_type, description)
    `)
    ...
    const shops = data.map((shop) => ({
        ...
        name: shop.name,
        logo: shop.logo_url,
        description: shop.software_integration?.[0]?.description || '',
